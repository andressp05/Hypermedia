...
exports.saleDbSetup = async function (database) {
  sqlDb = database;
  console.log("Checking if sales table exists");
  const exists = await sqlDb.schema.hasTable(Data.Tables.favorite);
  if (!exists) {
    console.log("sales doesn't esist so we need to create it");
  }
  else {
...
/**
 * Best sellers available in the inventory
 * List of best sellers available in the inventory
 *
...
exports.bestSellersGET = async function(offset = 0, limit = 6) {
  try {
    ...
    .from(Data.Tables.sale)
    .leftJoin(Data.Tables.book, Data.Tables.sale + '.ISBN', Data.Tables.book + '.ISBN')
    ...
    .groupBy(`${Data.Tables.book}.ISBN`).count('*').limit(limit).offset(offset);
